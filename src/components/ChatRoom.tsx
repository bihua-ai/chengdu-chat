...
export function ChatRoom({ roomId }: ChatRoomProps) {
  ...
  const [textareaHeight, setTextareaHeight] = useState(100); // Default height in pixels
  ...
  const handleMouseMove = (e: MouseEvent) => {
    ...
    setTextareaHeight((prev) => Math.min(Math.max(prev + delta, 100), 400)); // Min 100px, max 400px
    lastYRef.current = e.clientY;
  };
